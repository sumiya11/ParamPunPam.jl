...
#=
    ...
    3. Specialize the basis at a ``good'' point 
        and try to interpolate the coefficients.
    4. If the interpolation fails, go to 3.    
=#
function _paramgb(polys, metainfo)
    if metainfo.finite_field
        _paramgb_imodp(polys, metainfo)
    else
        _paramgb_rationals(polys, metainfo)
    end
end

mutable struct GroebnerState{Poly1, Smth1}
    originalpolys::Vector{Poly1}
    basisshape::Vector{Vector{Smth1}}
    coeffdegrees::Vector{Vector{Int}}
    function GroebnerState(polys::Vector{T}, metainfo) where {T}
        new(polys)
    end
end
...
function _paramgb_rationals(polys, metainfo)
    i = 0
    gbstate = GroebnerState(polys, metainfo)
    istate = InterpolatorState()
    discover_monomials!(gbstate)
    discover_degrees!(gbstate)
    npoints = 0
    while true
        reducemodp!(shapegb)
...
function discover_monomials!(shapegb::ShapeOfGb; η=2)
    ...
    bases = map(F -> groebner(Groebner.change_ordering(F, :degrevlex)), specializations)
