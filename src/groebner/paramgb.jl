...
- `up_to_degree` -- compute up to some fixed total degree of num./den., e.g., `up_to_degree=(2,2)`.
- `rational_interpolator` -- the rational function interpolation algorithm to use. Possible options are `CuytLee()` and `VanDerHoevenLecerf()`.
